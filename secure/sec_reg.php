...
*/
	...
	function checkDuplicate($db, $checkUsername, $checkEmail) 
	{
		...
		if($select_stmt = $db -> prepare("SELECT id FROM members WHERE username=? OR email=?")) 
		{
			//Bind parameters
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") 
	{
		...
		else 
		{
			...
			if ($insert_stmt = $dbcon->prepare("INSERT INTO members (username, email, password, salt) VALUES (?, ?, ?, ?)")) 
			{
